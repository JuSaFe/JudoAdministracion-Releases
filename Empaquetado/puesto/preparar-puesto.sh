...
#
#     sudo ./preparar-puesto.sh
#     sudo ./preparar-puesto.sh --deshacer
#
# Sin parámetros: busca el certificado del servidor (judo-server.crt) a su lado, en la carpeta desde
# la que se lanza y en el home. Lo normal es traerlo en un USB y ejecutar el guion desde esa carpeta.
#
# La aplicación se instala con su instalador (Documentación/00); esto es lo que queda después y no
# trae el instalador, porque depende de la red y del servidor concretos:
#
...
Prepara un puesto de administración para la competición (y lo deshace al terminar).
  ...
  sudo ./preparar-puesto.sh                  con el judo-server.crt al lado
  sudo ./preparar-puesto.sh --deshacer       devolver el equipo a como estaba

  --certificado RUTA   Certificado público del servidor (si no, se busca)
  ...
  --anfitrion          Este equipo es el que hostea la aplicación: apunta a localhost
                       en vez de al nombre del servidor. En el equipo servidor NO hace
                       falta: de eso se encarga preparar-servidor
...
# El home de quien lanza el guion, no el de root: es donde preparar-servidor deja la copia del
# certificado, y es de donde se coge cuando el puesto es el propio servidor.
if [[ -n "${SUDO_USER:-}" ]]; then
...
instalar_certificado() {
    if [[ "$SISTEMA" == "Darwin" ]]; then
        ...
        fallo "No sé dónde instalar certificados en este sistema. Hazlo a mano: guía §4.2."
    fi
}
...
# ── Archivo hosts ─────────────────────────────────────────────────────────────────────────────────
# La misma lógica y la misma marca que configurar-red.sh y que preparar-servidor.sh, a propósito:
# cada guion se puede usar por separado, y quitar la línea con cualquiera de ellos deja el archivo
# igual.
...
if [[ -z "$CERTIFICADO" ]]; then
    # Por orden de lo más probable: la carpeta desde la que se lanza (el USB), la del propio guion,
    # y el home —donde preparar-servidor deja la copia—.
    for candidato in \
        "./$NOMBRE_SERVIDOR.crt" \
        "./judo-server.crt" \
        "$AQUI/$NOMBRE_SERVIDOR.crt" \
        "$HOGAR/$NOMBRE_SERVIDOR.crt"
    do
...
[[ -n "$CERTIFICADO" && -f "$CERTIFICADO" ]] || fallo "No encuentro el certificado del servidor.
     Cópialo desde el servidor ($NOMBRE_SERVIDOR.crt, NO el .pfx) a esta carpeta, o indícalo
     con --certificado <ruta>. El servidor lo deja en el home de quien lanzó preparar-servidor."
...
if grep -q "DNS:$NOMBRE_SERVIDOR" <<<"$nombres"; then
    ...
    aviso "la aplicación rechazará la conexión; hay que reemitirlo (guía §3.4)"
...
if buscar_app; then
    ...
    aviso "el certificado se puede instalar igual; la aplicación, después (guía §4.1)"
fi
...
# estado del puesto es explícito: se puede leer qué tiene configurado sin deducirlo, y --deshacer
# sabe qué quitar. Lo que no lleva nunca es cadena de conexión: un puesto de la red no habla con
# PostgreSQL.
    ...
    cat > "$DIR_APP/appsettings.Local.json" <<JSON
{
    "//": [
        ...
        "ConnectionString vacía: un puesto de la red NO habla con PostgreSQL, todo va por la API.",
...
JSON
    ...
    if [[ $ANFITRION -eq 1 ]]; then
        aviso "este equipo es anfitrión pero NO es el servidor: las pantallas que aún no han pasado"
        aviso "por la API necesitan cadena de conexión, y desde aquí no hay ninguna que poner (guía §5)"
    fi
fi
...
# lugar de dejar un "no conecta" genérico. Es la lista de la guía §6.
